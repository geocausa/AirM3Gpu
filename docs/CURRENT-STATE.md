...
E119 closes the ordinary CL constructor/priority source chain without changing Linux.  passes queue  and the pre-increment CL-workqueue index ; matching IOGPU/AGX workqueue constructors preserve them at , and  receives them as its second/first integer arguments. Therefore SKU  is exactly the CL-workqueue index (0 for the first channel) and the normal-J615 second integer is . The priority setter tuple is ; the accelerator remap table is exact-zero on normal J615. Runtime foreground/QoS remains an explicit host input rather than a guessed constant.

...
## E048-E051 — worker/event handshake is not the stall

E048 correlated host-visible q21 state with the `FUN_3d330` cold-power transaction. The first signed Compute publication begins at `busy=0, unk10=0, ready=1, power=0`; after callback entry, firmware publishes `unk10=1, power=1` while `busy` remains zero. Thus the cold-power path reaches its public active state without entering `FUN_6a0c`.

E049 demonstrated an instrumentation pitfall: broad `0x20b/0x20c` promotion produced a multi-million-record class-2 log storm. E050 removed generic per-record KTrace output and kept only bounded decision IDs. The clean E050 run showed callback entry `0x100`, pstate `0x200`, and stats tag `0x0f/state=1` with the same state-transition timestamp; no `0x101` callback exit appeared.
